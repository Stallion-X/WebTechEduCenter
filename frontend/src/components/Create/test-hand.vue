<template>
  <div>
    ...
    <el-button type="text" @click="openDialog()"
      >添加题目</el-button
    >
    <el-dialog title="添加考题" :visible.sync="dialogTableVisible"
...
export default {
  ...
  methods: {
    ...
    openDialog() {
      ...
      this.axios
        .get("http://127.0.0.1:8080/getItemInfoLite", {
          params: { teacher_id: this.loginData.teacher_id, item_type: 2 },
        })
        .then((response) => {
    ...
    submit() {
      ...
      composeData.append("compose_type", 2);
