<template>
  <div>
    ...
    >
      <el-menu-item
        index="/one"
        :route="{ path: '/one', query: { item_type: 0 } }"
        >新建习题</el-menu-item
      >
      <el-submenu index="1">
        <template slot="title">新建作业</template>
        <el-menu-item index="/workh">手动创建</el-menu-item>
        <el-menu-item index="/worka">自动创建</el-menu-item>
      </el-submenu>
    </el-menu>
    <br /><br />
...
export default {
  ...
  methods: {
    handleClick(row) {
      console.log(row);
      this.$router.push({ path: "viewWork", query: { no: row.no } });
    },
